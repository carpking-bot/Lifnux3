...
function allDateKeysBetween(startKey, endKey) {
    const keys = [];
    let cursor = startKey;
    while (cursor <= endKey) {
        keys.push(cursor);
...
function estimateCalories(log) {
    if (typeof log.calorieOverride === "number" && log.calorieOverride > 0)
        return log.calorieOverride;
    const typeId = log.typeId;
    if (typeId === "running")
        return Math.max(0, (log.distanceKm ?? 0) * 80);
    if (typeId === "walking")
        return Math.max(0, (log.distanceKm ?? 0) * 40);
    if (typeId === "bicycle")
        return Math.max(0, (log.distanceKm ?? 0) * 30);
    if (typeId === "test_distance")
        return Math.max(0, (log.distanceKm ?? 0) * 60);
    if (typeId === "swimming")
        return 320;
    if (typeId === "test_count")
        return 300;
    if (typeId === "home")
        return Math.max(0, ((log.durationMin ?? 30) / 30) * 280);
    if (typeId === "soccer")
        return 600;
    if (typeId === "tennis")
        return Math.max(0, ((log.durationMin ?? 60) / 60) * 580);
    return 0;
}
export function calculateLogXP(log) {
    return Math.floor(estimateCalories(log) / 10);
}
...
function thresholdReachedDateForCount(logs, threshold) {
    if (threshold <= 0)
        return undefined;
...
function thresholdReachedDateForDistance(logs, thresholdKm) {
    if (thresholdKm <= 0)
        return undefined;
    let sum = 0;
...
export function calculateDecay(rawDailyXp, baseDateKey = todayDateKey()) {
    if (!rawDailyXp.length)
        return { finalXp: 0, totalDecayApplied: 0, missedDays: 0 };
    const map = new Map();
    for (const item of rawDailyXp)
        map.set(item.dateKey, (map.get(item.dateKey) ?? 0) + item.xp);
...
export function calculateTotalXP(logs, baseDateKey = todayDateKey()) {
...
function minXpForLevel(level, levelTable) {
    ...
    let prev = table[9];
    for (let lv = 11; lv <= level; lv += 1)
        prev += lv * 300;
    return prev;
}
export function calculateLevel(xp, levelTable) {
    let level = 1;
    while (minXpForLevel(level + 1, levelTable) <= xp)
        level += 1;
...
function specialBadgeForActivity(typeId, count, distance) {
    if (typeId === "soccer")
        return [{ id: "special-soccer", name: "\uBC1C\uB871\uB3C4\uB974", description: "\uCD95\uAD6C 100\uD68C \uB2EC\uC131", unlocked: count >= 100, tier: "special" }];
    if (typeId === "tennis")
        return [{ id: "special-tennis", name: "\uADF8\uB79C\uB4DC \uC2AC\uB7A8", description: "\uD14C\uB2C8\uC2A4 100\uD68C \uB2EC\uC131", unlocked: count >= 100, tier: "special" }];
    if (typeId === "swimming")
        return [{ id: "special-swim", name: "\uC778\uAC04 \uBB3C\uACE0\uAE30", description: "\uC218\uC601 100\uD68C \uB2EC\uC131", unlocked: count >= 100, tier: "special" }];
    if (typeId === "running")
        return [{ id: "special-running", name: "\uC11C\uC6B8 to \uBD80\uC0B0", description: "\uB7EC\uB2DD 400km \uB2EC\uC131", unlocked: distance >= 400, tier: "special" }];
    if (typeId === "walking")
        return [{ id: "special-walk", name: "\uD55C\uAC15 \uC885\uC8FC", description: "\uAC77\uAE30 300km \uB2EC\uC131", unlocked: distance >= 300, tier: "special" }];
    if (typeId === "home")
        return [{ id: "special-home", name: "\uC0AC\uC774\uD0C0\uB9C8", description: "\uD648 \uD2B8\uB808\uC774\uB2DD 100\uD68C \uB2EC\uC131", unlocked: count >= 100, tier: "special" }];
    return [];
}
function activityLabelKo(typeId) {
    if (typeId === "running")
        return "\uB7EC\uB2DD";
    if (typeId === "walking")
        return "\uAC77\uAE30";
...
function applyBadgeTextTemplate(template, vars) {
    if (!template)
        return "";
    let text = template;
...
export function generateBadges(logs, selectedTypeId, baseDateKey = todayDateKey()) {
    ...
    const countStep = typeId === "swimming" || typeId === "home" || typeId === "gym" || typeId === "test_count" ? 10 : typeId === "soccer" || typeId === "tennis" ? 5 : 0;
    if (countStep > 0) {
...
export function generateBadgesByRules(logs, selectedTypeId, badgeRules, baseDateKey = todayDateKey()) {
    if (!badgeRules || !badgeRules.length) {
        return generateBadges(logs, selectedTypeId, baseDateKey);
    }
    const streak = calculateStreak(logs, baseDateKey);
    const months = perfectMonths(logs);
    const globalBadges = [];
    const activityBadges = [];
    const push = (scope, item) => {
        if (scope === "global")
            globalBadges.push(item);
        else
            activityBadges.push(item);
    };
    for (const rule of badgeRules) {
        const targetActivity = rule.activity || selectedTypeId;
        if (rule.scope === "activity" && targetActivity !== selectedTypeId)
            continue;
    ...
    const firstLogBadgeTypes = new Set(activityBadges

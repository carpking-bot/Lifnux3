...
function toFormState(typeId, log, initialDateKey) {
    if (!log) {
        return {
...
export function LogEditorModal({ open, selectedType, editingLog, initialDateKey, onClose, onSubmit, onNotify }) {
    const [form, setForm] = useState(() => toFormState(selectedType.id, editingLog, initialDateKey));
    useEffect(() => {
        if (!open)
            return;
    ...
    return (<PlainModal open={open} title={editingLog ? `Edit ${selectedType.name} Log` : `Log ${selectedType.name} Workout`} onClose={onClose} actions={<>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-xs text-[var(--ink-1)]">
          Date
          <input type="date" className={fieldClassName()} value={form.loggedForDate} onChange={(event) => setForm((prev) => ({ ...prev, loggedForDate: event.target.value }))}/>
        </label>
        <label className="text-xs text-[var(--ink-1)]">
          Duration (min)
          <input type="number" min={0} className={fieldClassName()} value={form.durationMin} onChange={(event) => setForm((prev) => ({ ...prev, durationMin: event.target.value }))}/>
        </label>
        <label className="text-xs text-[var(--ink-1)] sm:col-span-2">
          Calorie Override
          <input type="number" min={1} className={fieldClassName()} value={form.calorieOverride} onChange={(event) => setForm((prev) => ({ ...prev, calorieOverride: event.target.value }))}/>
        </label>
        <label className="text-xs text-[var(--ink-1)] sm:col-span-2">
          Memo
          <textarea rows={3} className={fieldClassName()} value={form.memo} onChange={(event) => setForm((prev) => ({ ...prev, memo: event.target.value }))}/>
        </label>
        {isDistanceType ? (<>
            <label className="text-xs text-[var(--ink-1)]">
              Distance (km)
              <input type="number" step="0.1" min="0.1" className={fieldClassName()} value={form.distanceKm} onChange={(event) => setForm((prev) => ({ ...prev, distanceKm: event.target.value }))}/>
            </label>
            {selectedType.id === "running" ? (<>
                <label className="text-xs text-[var(--ink-1)]">
                  Pace Text
                  <input type="text" placeholder={`5'10"`} className={fieldClassName()} value={form.paceText} onChange={(event) => setForm((prev) => ({ ...prev, paceText: event.target.value }))}/>
                </label>
                <label className="text-xs text-[var(--ink-1)]">
                  Cadence
                  <input type="number" className={fieldClassName()} value={form.cadence} onChange={(event) => setForm((prev) => ({ ...prev, cadence: event.target.value }))}/>
                </label>
                <label className="text-xs text-[var(--ink-1)]">
                  Max Speed (km/h)
                  <input type="number" step="0.1" className={fieldClassName()} value={form.maxSpeedKmh} onChange={(event) => setForm((prev) => ({ ...prev, maxSpeedKmh: event.target.value }))}/>
                </label>
              </>) : null}
          </>) : null}
      </div>

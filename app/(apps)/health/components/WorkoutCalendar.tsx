...
export function WorkoutCalendar({ monthKey, selectedDateKey, markedDateCounts, onChangeMonth, onSelectDate }: WorkoutCalendarProps) {
    ...
    <section className="rounded-xl border border-white/10 bg-black/15 p-3">
      ...
      <div className="grid grid-cols-7 gap-1">
        {weekdays.map((label, idx) => (
          <div
            key={label}
            className={`px-2 py-1 text-center text-[10px] uppercase tracking-[0.08em] ${
              idx >= 5 ? "text-rose-300" : "text-[var(--ink-1)]"
            }`}
          >
            {label}
          </div>
        ...
        {cells.map((cell) => {
          ...
          return (
            <button
              key={cell.dateKey}
              className={`relative min-h-10 rounded-md border px-1 py-1 text-xs transition ${stateClass}`}

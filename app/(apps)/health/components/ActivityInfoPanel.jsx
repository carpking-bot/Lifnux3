...
function logMeta(log) {
    const items = [];
    if (typeof log.durationMin === "number")
        items.push(`${log.durationMin} min`);
    ...
    if (log.typeId === "running" && log.paceText)
...
export function ActivityInfoPanel({ selectedType, selectedDateKey, calendarMonthKey, markedDateCounts, weeklyCount, weeklyTarget, monthlyCount, runningWeeklyKm, runningMonthlyKm, recentLogs, onAddLog, onEditTarget, onChangePlanMode, onCalendarMonthChange, onCalendarDateSelect, onOpenRunningGame, onEditLog, onDeleteLog }) {
    ...
    return (<section className="rounded-2xl border border-white/10 bg-[#111823] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <select className="lifnux-select rounded-full border border-white/15 px-3 py-1.5 text-[10px] uppercase tracking-[0.08em] text-[var(--ink-1)]" value={selectedType.planMode} onChange={(event) => onChangePlanMode(event.target.value)}>
          ...
          <div className="mt-1 space-y-1 text-sm text-[var(--ink-1)]">
            {summaryItems.map((item) => (<div key={item}>{item}</div>))}
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {selectedType.planMode !== "unplanned" ? (<button className="rounded-full border border-white/15 px-4 py-2 text-xs text-[var(--ink-1)]" onClick={onEditTarget}>
              Edit Target
            </button>) : null}
          {selectedType.id === "running" ? (<button className="rounded-full border border-white/15 px-4 py-2 text-xs text-[var(--ink-1)]" onClick={onOpenRunningGame}>
              Open Running Game
            </button>) : null}
        </div>
      </div>
      ...
      <div className="mt-5">
        ...
        <WorkoutCalendar monthKey={calendarMonthKey} selectedDateKey={selectedDateKey} markedDateCounts={markedDateCounts} onChangeMonth={onCalendarMonthChange} onSelectDate={onCalendarDateSelect}/>
      </div>

      <div className="mt-5">
        <div className="mb-2 text-xs uppercase tracking-[0.2em] text-[var(--ink-1)]">Recent Logs</div>
        {recentLogs.length === 0 ? (<div className="rounded-xl border border-white/10 bg-black/20 px-4 py-6 text-sm text-[var(--ink-1)]">No logs yet.</div>) : (<div className="space-y-2">
            {recentLogs.map((log) => (<div key={log.id} className="rounded-xl border border-white/10 bg-black/20 p-3">

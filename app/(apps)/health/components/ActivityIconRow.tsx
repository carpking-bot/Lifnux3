...
import type { ActivityType, ActivityTypeId } from "../types";
import { resolveActivityIcon } from "../lib/icon";

type ActivityIconRowProps = {
  types: ActivityType[];
  selectedTypeId: ActivityTypeId;
  onSelect: (typeId: ActivityTypeId) => void;
};

export function ActivityIconRow({ types, selectedTypeId, onSelect }: ActivityIconRowProps) {
  ...
  return (
    <section className="rounded-2xl border border-[#223248] bg-[#0f1725] p-4">
        ...
        {types.map((type) => {
          ...
          return (
            <button
              ...
              className={`min-w-[152px] rounded-xl border px-3 py-4 text-center transition ${
                selected

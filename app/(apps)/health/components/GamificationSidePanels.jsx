...
function flameVisualClass(level) {
    if (level >= 10)
        return "h-52 w-52 drop-shadow-[0_0_28px_rgba(251,191,36,0.55)]";
    if (level >= 7)
        return "h-44 w-44 drop-shadow-[0_0_24px_rgba(34,211,238,0.5)]";
    if (level >= 5)
        return "h-36 w-36 drop-shadow-[0_0_22px_rgba(253,186,116,0.45)]";
    if (level >= 3)
        return "h-[7.5rem] w-[7.5rem] drop-shadow-[0_0_18px_rgba(251,146,60,0.4)]";
    return "h-24 w-24";
}
export function BadgeShowcasePanel({ logs, selectedTypeId }) {
    ...
    return (<aside className="rounded-2xl border border-white/10 bg-[#111823] p-4 xl:sticky xl:top-24">
      <div className="mb-3 text-xs uppercase tracking-[0.2em] text-[var(--ink-1)]">Badges</div>
      {unlockedBadges.length === 0 ? (<div className="rounded-xl border border-white/10 bg-black/20 px-3 py-4 text-xs text-[var(--ink-1)]">
          ...
          {unlockedBadges.map((badge) => {
            ...
            return (<div key={badge.id} title={`${displayName}\n${displayDesc}${badge.achievedDate ? `\n획득일: ${badge.achievedDate}` : ""}`} className={`group relative flex h-[88px] w-[88px] items-center justify-center rounded-xl border ${badge.tier === "special"
...
export function LevelFlamePanel({ logs }) {
    ...
    return (<aside className="bg-transparent p-2 xl:sticky xl:top-24">
      <div className="mx-auto flex min-h-[820px] w-full flex-col items-end justify-center pr-4">
        <div className="translate-x-4">
          <div className="group relative flex w-[220px] justify-center">
            ...
            {hoverMeta ? (<div className="pointer-events-none absolute bottom-full right-0 z-20 mb-2 hidden w-48 rounded-lg border border-white/15 bg-[#0b1220] p-2 text-xs text-[var(--ink-0)] group-hover:block">

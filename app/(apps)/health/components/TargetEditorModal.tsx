...
type TargetEditorModalProps = {
  ...
  weekLabel: string;
  onClose: () => void;
  onSave: (nextTarget: number) => void;
  onNotify: (message: string) => void;
};

export function TargetEditorModal({ open, selectedType, currentTarget, weekLabel, onClose, onSave, onNotify }: TargetEditorModalProps) {
  ...
  return (
    <PlainModal
      open={open}
      title={`Edit ${selectedType.name} ${selectedType.planMode === "monthly" ? "Monthly" : "Weekly"} Target`}
      onClose={onClose}
      actions={
        <>
    ...
    >
      <label className="text-xs text-[var(--ink-1)]">
        ...
        <input
          type="number"
          min={0}
          max={selectedType.planMode === "monthly" ? 60 : 14}
          step={1}
      ...
      {selectedType.planMode === "weekly" ? (
        <div className="mt-2 text-xs text-[var(--ink-1)]">Applied week: {weekLabel}</div>
      ) : null}
    </PlainModal>
  );
}

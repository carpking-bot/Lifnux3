...
export function buildLifnuxExport() {
    const data = {};
    ...
    for (let i = 0; i < window.localStorage.length; i += 1) {
        ...
        if (!key || !isLifnuxKey(key) || key === BACKUP_KEY)
            continue;
...
export function getBackupExport() {
    if (typeof window === "undefined")
        return null;
...
export function downloadLifnuxExport({ useBackup = false } = {}) {
    if (typeof window === "undefined")
        return;
...
export function validateLifnuxExport(payload) {
    if (!payload || typeof payload !== "object")
        return false;
    const candidate = payload;
    return candidate.meta?.app === "lifnux" && typeof candidate.data === "object";
}
export function importLifnuxExport(payload) {
    if (typeof window === "undefined")
        return;
    const keysToClear = [];
    for (let i = 0; i < window.localStorage.length; i += 1) {
        const key = window.localStorage.key(i);
        if (key && isLifnuxKey(key))
